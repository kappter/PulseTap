...
function buildArrangementFile() {
  ...
  return {
    ...
    // roleMap: role -> [loopId, ...]
    roleMap: loopLibrary.reduce((acc, loop) => {
      const role = loop.role || "Unassigned";
      if (!acc[role]) acc[role] = [];
      acc[role].push(loop.loopId);
      return acc;
    }, {})
  };
}
...
function importArrangement(data) {
  if (!data || !["1.0", "1.1", "1.2"].includes(data.ptarrVersion)) {
...
/** Wire the Export WAV Mix button */
const exportWavBtn = document.createElement("button");
exportWavBtn.id = "exportWavBtn";
exportWavBtn.className = "arr-btn";
exportWavBtn.textContent = "Export WAV Mix";
exportWavBtn.addEventListener("pointerdown", (e) => {
  e.preventDefault();
  renderArrangementToWav();
});

// Inject the WAV button next to the Export Arrangement button
document.addEventListener("DOMContentLoaded", () => {
  const btnGroup = document.querySelector(".arr-file-btns");
  if (btnGroup) {
    btnGroup.appendChild(exportWavBtn);
  }
});
...
async function renderArrangementToWav() {
  if (!arrangementSections || arrangementSections.length === 0) {
    log("No sections to render.", "error");
    return;
  }

  log("Rendering arrangement to WAV...", "system");
  ...
  arrangementSections.forEach(sec => {
    totalDurationSec += sec.bars * secPerBar;
  });
  
  // Add a 1-second tail for reverb/release
  ...
  // We temporarily override the global hostAudioCtx so the synth functions use the offline context
  const originalCtx = hostAudioCtx;
  const originalGain = hostMasterGain;
  hostAudioCtx = offlineCtx;
  hostMasterGain = offlineMasterGain;
  ...
  arrangementSections.forEach(sec => {
    ...
    assignedIds.forEach(loopId => {
      ...
      if (!loopData) return;
      const events = getEventsFromLoop(loopData);
      const loopLenSec = (loopData.loopLengthMs || (secPerBar * 1000)) / 1000;
      const loopsNeeded = Math.ceil(sectionDurSec / loopLenSec);

      for (let i = 0; i < loopsNeeded; i++) {
        const loopOffsetSec = i * loopLenSec;
        events.forEach(ev => {
          const evAcTime = currentAcSec + loopOffsetSec + (ev.timeMs / 1000);
          if (evAcTime >= currentAcSec && evAcTime < currentAcSec + sectionDurSec) {
            playHostSound(ev.degree, ev.instrument, evAcTime);
          }
        });
      }
  ...
  try {
    ...
    const a = document.createElement("a");
    const title = (currentRoom ? `pulsetap_room_${currentRoom}` : "pulsetap_arrangement").replace(/[^a-z0-9_-]/gi, "_").toLowerCase();
    a.href = url;
    a.download = `${title}_mix.wav`;
    document.body.appendChild(a);
    a.click();
    setTimeout(() => { URL.revokeObjectURL(url); a.remove(); }, 1000);
    
    log("WAV export complete.", "system");
  } catch (err) {
    log("WAV export failed: " + err.message, "error");
  } finally {
    // Restore the live audio context
    hostAudioCtx = originalCtx;
    hostMasterGain = originalGain;
    if (exportBtn) exportBtn.textContent = "Export WAV Mix";
  }
}
